...
class DatasetLoader(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 1. load image
        cvimg = cv2.imread(data['img_path'], cv2.IMREAD_UNCHANGED | cv2.IMREAD_IGNORE_ORIENTATION)
        ...
        if self.do_augment:
            ...
            scale, rot, do_flip, color_scale, do_occlusion = 1.0, 0.0, False, [1.0, 1.0, 1.0, 1.0], False
...
### CHANGED THIS TO LEN -1!!!!!!!!!!!
        for i in range(len(joint_img)-1):
    ...
    if do_occlusion:
        while True:
            ...
            if synth_xmin >= 0 and synth_ymin >= 0 and synth_xmin + synth_w < img_width and synth_ymin + synth_h < img_height:
                ...
                w = int(synth_w)
                h = int(synth_h)
                img[ymin:ymin+h, xmin:xmin+w, :] = np.random.rand(h, w, 4) * 255
                break
