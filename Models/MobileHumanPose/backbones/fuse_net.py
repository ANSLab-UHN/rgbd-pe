...
class fuseNet(nn.Module):
    def __init__(self,
        ...
        self.big_conv = nn.Conv2d(in_channels=320, out_channels=320, kernel_size=3, stride=1, padding=1)
        self.last_conv_fuse = nn.Conv2d(in_channels=640, out_channels=320, kernel_size=3, stride=1, padding=1)
        self.f1_conv = nn.Conv2d(in_channels=960, out_channels=960, kernel_size=3, stride=1, padding=1)
        self.f2_conv = nn.Conv2d(in_channels=960, out_channels=320, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        # TIME TO FUSE BOTH ENCODER OUTPUTS!!!
# FUSION TECHNIQUE
        xm = xr * xd                        # torch.Size([32, 320, 4, 4])
        x3 = torch.cat((xr, xd, xm), 1)     # torch.Size([32, 960, 4, 4])
        x3 = self.f1_conv(x3)               # torch.Size([32, 960, 4, 4]) 
        x3 = self.act(x3)                   # torch.Size([32, 960, 4, 4])
        x3 = self.f2_conv(x3)               # torch.Size([32, 320, 4, 4])
        x3 = self.act(x3)                   # torch.Size([32, 320, 4, 4])

        # OUTER BRANCHES
        xr = self.big_conv(xr)              # torch.Size([32, 320, 4, 4])
        xr = xr + x3                        # torch.Size([32, 320, 4, 4])
        xd = self.big_conv(xd)              # torch.Size([32, 320, 4, 4]) 
        xd = xd + x3                        # torch.Size([32, 320, 4, 4])

            # REP 2
        xm = xr * xd
        x3 = torch.cat((xr, xd, xm), 1)
        x3 = self.f1_conv(x3)
        x3 = self.act(x3)
        x3 = self.f2_conv(x3)
        x3 = self.act(x3)
        ...
        x = self.last_conv_fuse(x)          # torch.Size([32, 320, 4, 4])
    ...
    def init_weights(self):
        ...
        for j in [self.first_conv_r, self.first_conv_d, self.big_conv, self.last_conv_fuse, self.f1_conv, self.f2_conv, self.inv_residual, self.last_conv, self.final_layer]:
        #for j in [self.first_conv_r, self.first_conv_d, self.big_conv, self.last_conv_fuse, self.inv_residual, self.last_conv, self.final_layer]:
...
if __name__ == "__main__":
    fuseNet((256, 256), 18).init_weights()
    model = fuseNet((256, 256), 18)
    model.to('cuda')
