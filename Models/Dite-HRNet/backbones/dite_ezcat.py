...
class ezCAT_DiteHRNet(nn.Module):
    ...
    def __init__(self,
        ...
        #self.fix_conv = nn.Conv2d(in_channels=80, out_channels=40, kernel_size=1, stride=1, padding=0)      # 1x1 convolution to bring the number of channels back down to 40
    ...
    def forward(self, x):
        ...
        # So far so good here, we got two different stems, one for 
        x1 = self.stem1(x1)         # torch.Size([8, 32, 96, 72])
        x2 = self.stem2(x2)         # torch.Size([8, 32, 96, 72])
        ...
        x1 = x1[0]                  # torch.Size([8, 40, 96, 72]), first dim is the batch size, then the channels, then the height and the width
        x2 = x2[0]                  # torch.Size([8, 40, 96, 72])

        # COMBINE THE TWO - CONCATENATION METHOD
        x = torch.cat((x1, x2), 1)  # torch.Size([8, 80, 96, 72])             
        #x = self.fix_conv(x)        # torch.Size([8, 40, 96, 72])
        
        
        # Combine the two - essentially average pooling technique.
        #x = (x1 + x2)/2              # torch.Size([8, 40, 96, 72])
