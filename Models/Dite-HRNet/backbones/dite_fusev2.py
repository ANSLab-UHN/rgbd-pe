# Correct architecture as of April 16
...
class Fusev2_DiteHRNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.big_conv = nn.Conv2d(in_channels=40, out_channels=40, kernel_size=3, stride=1, padding=1)
        self.last_conv = nn.Conv2d(in_channels=80, out_channels=40, kernel_size=3, stride=1, padding=1)
        self.f1_conv = nn.Conv2d(in_channels=120, out_channels=120, kernel_size=3, stride=1, padding=1)
        self.f2_conv = nn.Conv2d(in_channels=120, out_channels=40, kernel_size=3, stride=1, padding=1)
        self.act = nn.ReLU(inplace=True)

        self.stem1 = Stem(      # For the RGB input
            3,
            stem_channels=self.extra['stem']['stem_channels'],
            out_channels=self.extra['stem']['out_channels'],
            with_cp=with_cp)
        
        self.stem2 = Stem(      # For the depth input
            1,
            stem_channels=self.extra['stem']['stem_channels'],
            out_channels=self.extra['stem']['out_channels'],
            with_cp=with_cp)
    ...
    def forward(self, x):
        ...
        x1 = x[:, :3, :, :]         # First 3 channels = color
        x2 = x[:, -1:, :, :]        # Last channel = depth
        # at this point: x1.shape = torch.Size([8, 3, 384, 288])
        #                x2.shape = torch.Size([8, 1, 384, 288])

        # So far so good here, we got two different stems, one for rgb and the other for depth
        x1 = self.stem1(x1)
        x2 = self.stem2(x2)
        ...
        for i in range(self.num_stages):
            ...
            for j in range(self.stages_spec['num_branches'][i]):
                if transition[j]:
                    if j >= len(y1_list):
                        x1_list.append(transition[j](y1_list[-1]))
                    else:
                        x1_list.append(transition[j](y1_list[j]))
                else:
                    x1_list.append(y1_list[j])
            y1_list = getattr(self, 'stage{}'.format(i))(x1_list)
        
        # Repeating it again to go through the depth stuff
        for i in range(self.num_stages):
            x2_list = []
            transition = getattr(self, 'transition{}'.format(i))
            for j in range(self.stages_spec['num_branches'][i]):
                if transition[j]:
                    if j >= len(y2_list):
                        x2_list.append(transition[j](y2_list[-1]))
                    else:
                        x2_list.append(transition[j](y2_list[j]))
                else:
                    x2_list.append(y2_list[j])
            y2_list = getattr(self, 'stage{}'.format(i))(x2_list)
        ...
        if self.with_head:
            ...
            x2 = self.head_layer(x2)
        
        
        x1 = x1[0]                  # torch.Size([8, 40, 96, 72])
        x2 = x2[0]                  # torch.Size([8, 40, 96, 72])
     
    
        # FUSION BLOCK
        xm = x1 * x2                        # torch.Size([8, 40, 96, 72])
        #xm = torch.einsum('abcd, abcd -> abcd', x1, x2)
        x3 = torch.cat((x1, x2, xm), 1)     # torch.Size([8, 120, 96, 72])
        x3 = self.f1_conv(x3)               # torch.Size([8, 120, 96, 72])
        x3 = self.act(x3)                   # torch.Size([8, 120, 96, 72])
        x3 = self.f2_conv(x3)               # torch.Size([8, 40, 96, 72])
        x3 = self.act(x3)                   # torch.Size([8, 40, 96, 72])

        # OUTER BRANCHES
        x1 = self.big_conv(x1)      # torch.Size([8, 40, 96, 72])
        x1 = x1 + x3                # torch.Size([8, 40, 96, 72])
        x2 = self.big_conv(x2)      # torch.Size([8, 40, 96, 72])
        x2 = x2 + x3                # torch.Size([8, 40, 96, 72])

            # REP 2
        xm = x1 * x2
        #xm = torch.einsum('abcd, abcd -> abcd', x1, x2)
        x3 = torch.cat((x1, x2, xm), 1)
        x3 = self.f1_conv(x3)
        x3 = self.act(x3)
        x3 = self.f2_conv(x3)
        x3 = self.act(x3)
        ...
        # FINAL STEPS
        x = torch.cat((x1, x2), 1)      # torch.Size([8, 80, 96, 72])
        x = self.last_conv(x)           # torch.Size([8, 40, 96, 72])
